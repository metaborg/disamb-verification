...
Fixpoint yield {L O} (t : parse_tree L O) : word L O :=
  match t with
  | ANode lex => [inl lex]
...
Definition matches_set {L O} (t : parse_tree L O) (qs : tree_pattern O -> Prop) :=
  exists q, qs q /\ matches t q.

(* valid qs t states that a parse tree and all its subtrees should not match
   any patterns in qs. *)
Inductive valid {L O} (qs : tree_pattern O -> Prop) : parse_tree L O  -> Prop :=
  | ANode_valid lex :
...
Inductive cpatterns {O} (g : dgrammar O) : tree_pattern O -> Prop :=
  | IL o1 o2 :
