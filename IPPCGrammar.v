...
Definition OP g := T g : Type.

Definition word g := list (OP g).

Inductive parse_tree (g : ipg) :=
  | AtomicNode : OP g -> parse_tree g
  ...
  | ClosedNode : OP g → parse_tree g → OP g → parse_tree g.
...
Global Arguments ClosedNode {_} _ _ _.

Inductive wf_parse_tree g : parse_tree g -> Prop :=
  | Atomic_wf l :
      g.(prods) (AtomicProd l) →
  ...
  | Closed_wf c1 t c2 :
      g.(prods) (ClosedProd c1 c2) →
      wf_parse_tree g t →
      wf_parse_tree g (ClosedNode c1 t c2).

Fixpoint yield {g} t : word g :=
  match t with
  | AtomicNode l => [l]
  | InfixNode t1 o t2 => yield t1 ++ o :: yield t2
  | PrefixNode o t => o :: yield t
  | PostfixNode t o => yield t ++ [o]
  | ClosedNode a1 t a2 => a1 :: yield t ++ [a2] 
  end.
...
Inductive i_conflict_free {g} (Q : tree_pattern g -> Prop) : parse_tree g -> Prop :=
  ...
  | Closed_cf a1 t a2 :
      i_conflict_free Q t →
      i_conflict_free Q (ClosedNode a1 t a2).
...
Inductive drm_conflict_free {g} (Q : tree_pattern g -> Prop) : parse_tree g -> Prop :=
  ...
  | Closed_drmcf a1 t a2 :
      drm_conflict_free Q t →
      drm_conflict_free Q (ClosedNode a1 t a2).
...
Inductive dlm_conflict_free {g} (Q : tree_pattern g -> Prop) : parse_tree g -> Prop :=
  ...
  | Closed_dlmcf a1 t a2 :
      dlm_conflict_free Q t →
      dlm_conflict_free Q (ClosedNode a1 t a2).
...
Fixpoint repair {g} (pr : drules g) t : parse_tree g :=
  ...
  | ClosedNode a1 t a2 => ClosedNode a1 (repair pr t) a2
  end.

End IPPCGrammarRepair.
