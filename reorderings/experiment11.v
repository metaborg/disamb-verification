...
Inductive production Terminal :=
  ...
  | PrefixProduction : Terminal → list Terminal → production Terminal
  | PostfixProduction : Terminal → list Terminal → production Terminal.

Global Arguments ClosedProduction {_} _ _.
Global Arguments InfixProduction {_} _ _.
Global Arguments PrefixProduction {_} _ _.
Global Arguments PostfixProduction {_} _ _.
...
Notation ClosedP a ac := (P (ClosedProduction a ac)).
Notation InP a ac := (P (InfixProduction a ac)).
Notation PreP a ac := (P (PrefixProduction a ac)).
Notation PostP a ac := (P (PostfixProduction a ac)).
...
Inductive parse_tree :=
  ...
  | PrefixNode : T → parse_list → parse_tree → parse_tree
  | PostfixNode : parse_tree → T → parse_list → parse_tree
with parse_list :=
  | NilNode : parse_list
...
Inductive well_formed_parse_tree : PT → Prop :=
  ...
  | WellFormedPrefixNode a ac τ t :
      PreP a ac →
      well_formed_parse_list ac τ →
      well_formed_parse_tree t →
      well_formed_parse_tree (PeN a τ t)
  | WellFormedPostfixNode τ t a ac :
      PostP a ac →
      well_formed_parse_tree t →
      well_formed_parse_list ac τ →
      well_formed_parse_tree (PoN t a τ)
...
Fixpoint yield t : word :=
  ...
  | PeN a τ t => a :: yield_list τ ++ yield t
  | PoN t a τ => yield t ++ a :: yield_list τ
  end
...
Inductive reorder_step : PT → PT → Prop :=
  ...
  | ReorderStepInfixPrefix t1 t2 τ1 τ2 a b :
      reorder_step (IN (PeN a τ1 t1) b τ2 t2) (PeN a τ1 (IN t1 b τ2 t2))
  | ReorderStepInfixPostfix t1 t2 τ1 τ2 a b :
      reorder_step (IN t1 a τ1 (PoN t2 b τ2)) (PoN (IN t1 a τ1 t2) b τ2)
  | ReorderStepPrefixPostfix t τ1 τ2 a b :
      reorder_step (PeN a τ1 (PoN t b τ2)) (PoN (PeN a τ1 t) b τ2)
  ...
  | ReorderStepInfixSubtree3 t1 a τ t2 τ' :
      ...
      reorder_step (IN t1 a τ t2) (IN t1 a τ' t2)
  | ReorderStepPrefixSubtree1 a τ t t' :
      reorder_step t t' →
      reorder_step (PeN a τ t) (PeN a τ t')
  | ReorderStepPrefixSubtree2 a τ t τ' :
      reorder_step_list τ τ' →
      reorder_step (PeN a τ t) (PeN a τ' t)
  | ReorderStepPostfixSubtree1 a τ t t' :
      reorder_step t t' →
      reorder_step (PoN t a τ) (PoN t' a τ)
  | ReorderStepPostfixSubtree2 a τ t τ' :
      reorder_step_list τ τ' →
      reorder_step (PoN t a τ) (PoN t a τ') 
...
Lemma reorder_prefix_subtree1 o t τ t' :
  t ⟷* t' →
  (PeN o τ t) ⟷* (PeN o τ t').
Proof.
  intro. induction H.
  - apply rtc_refl.
  - apply rtc_l with (PeN o τ y).
    + inv H.
      * apply sc_lr. apply ReorderStepPrefixSubtree1. assumption.
      * apply sc_rl. apply ReorderStepPrefixSubtree1. assumption.
    + assumption.
Qed.

Lemma reorder_prefix_subtree2 o t τ τ' :
  τ <---->* τ' →
  (PeN o τ t) ⟷* (PeN o τ' t).
Proof.
  intro. induction H.
  - apply rtc_refl.
  - apply rtc_l with (PeN o y t).
    + inv H.
      * apply sc_lr. apply ReorderStepPrefixSubtree2. assumption.
      * apply sc_rl. apply ReorderStepPrefixSubtree2. assumption.
    + assumption.
Qed.

Lemma reorder_postfix_subtree1 τ t o t' :
  t ⟷* t' →
  (PoN t o τ) ⟷* (PoN t' o τ).
Proof.
  intro. induction H.
  - apply rtc_refl.
  - apply rtc_l with (PoN y o τ).
    + inv H.
      * apply sc_lr. apply ReorderStepPostfixSubtree1. assumption.
      * apply sc_rl. apply ReorderStepPostfixSubtree1. assumption.
    + assumption.
Qed.

Lemma reorder_postfix_subtree2 τ t o τ' :
  τ <---->* τ' →
  (PoN t o τ) ⟷* (PoN t o τ').
Proof.
  intro. induction H.
  - apply rtc_refl.
  - apply rtc_l with (PoN t o y).
    + inv H.
      * apply sc_lr. apply ReorderStepPostfixSubtree2. assumption.
      * apply sc_rl. apply ReorderStepPostfixSubtree2. assumption.
    + assumption.
Qed.
...
Inductive yield_struct : word → PT → Prop :=
  ...
  | PrefixYieldStruct a ac w wi t τ :
      PreP a ac →
      interleaving ac wi τ →
      yield_struct w t →
      yield_struct (a :: wi ++ w) (PeN a τ t)
...
with yield_some_struct : PT → word → PT → Prop :=
  ...
  | PostfixYieldStruct a ac wi wt t1 τ t :
      PostP a ac →
      interleaving ac wi τ →
      yield_some_struct (PoN t1 a τ) wt t →
      yield_some_struct t1 (a :: wi ++ wt) t.
...
Proof.
  - intros. inv H.
    ...
    + simpl. rewrite <- app_assoc.
      specialize yield_struct_infix_sound with w t wi τ w2 t2 a ac.
      destruct yield_struct_infix_sound; auto. rename x into t'.
      exists (PeN a0 τ0 t'). inv H. split.
      * eapply PrefixYieldStruct; eauto.
      * eapply rtc_l.
        **apply sc_lr. apply ReorderStepInfixPrefix.
        **apply reorder_prefix_subtree1. assumption.
  - intros. inv H.
    ...
    + simpl. rewrite <- app_assoc.
      edestruct yield_some_struct_infix_sound with (a := a) (ac := ac); eauto.
      inv H. rename x into t'. exists t'. split; auto.
      eapply PostfixYieldStruct; eauto.
Qed.

Lemma yield_struct_postfix_sound w1 wi τ t1 a ac :
  ys w1 t1 → il ac wi τ → PostP a ac →
  ∃ t', ys (w1 ++ a :: wi) t' ∧ (PoN t1 a τ) ⟷* t'
with yield_some_struct_postfix_sound ti wi τ w1 t1 a ac :
  yss ti w1 t1 → il ac wi τ → PostP a ac →
  ∃ t', yss ti (w1 ++ a :: wi) t' ∧ (PoN t1 a τ) ⟷* t'.
Proof.
  - intros. inv H.
    + simpl. rewrite <- app_assoc.
      edestruct yield_some_struct_postfix_sound with (a := a) (ac := ac); eauto.
      rename x into t'. inv H. exists t'. split; eauto. eapply ClosedYieldStruct; eauto.
    + simpl. rewrite <- app_assoc.
      edestruct yield_struct_postfix_sound with (a := a) (ac := ac); eauto. inv H.
      rename x into t'. exists (PeN a0 τ0 t'). split.
      * eapply PrefixYieldStruct; eauto.
      * eapply rtc_transitive. apply rtc_once.
        apply sc_rl. apply ReorderStepPrefixPostfix.
        apply reorder_prefix_subtree1. assumption.
  - intros. inv H.
    + simpl. exists (PoN t1 a τ). split.
      * replace wi with (wi ++ []); [|auto using app_nil_r].
        eapply PostfixYieldStruct; eauto. apply NilYieldStruct.
      * apply rtc_refl.
    + simpl. rewrite <- app_assoc.
      edestruct yield_struct_postfix_sound with (a := a) (ac := ac); eauto.
      ...
      * eapply rtc_transitive. apply rtc_once.
        ...
        apply reorder_infix_subtree2. assumption.
    + simpl. rewrite <- app_assoc.
      edestruct yield_some_struct_postfix_sound
        with (ti := (PoN ti a0 τ0)) (a := a) (ac := ac); eauto.
      inv H. rename x into t'.
      exists t'. split; eauto. eapply PostfixYieldStruct; eauto.
Qed.
...
Proof.
  - intros. inv H.
    ...
    + simpl. apply yield_struct_sound in H1. apply yield_struct_sound in H3.
      ...
      assumption.
    + simpl.
      apply yield_struct_sound in H2. destruct H2 as [t2']. inv H.
      apply interleave_sound in H1. destruct H1 as [τ']. inv H.
      exists (PeN a τ' t2'). split.
      * eapply PrefixYieldStruct; eauto.
      * eapply rtc_transitive.
        apply reorder_prefix_subtree1; eauto.
        eapply reorder_prefix_subtree2.
        assumption.
    + simpl. apply yield_struct_sound in H1. destruct H1 as [t1']. inv H.
      apply interleave_sound in H2. destruct H2 as [τ']. inv H.
      eapply yield_struct_postfix_sound in H1; eauto.
      destruct H1 as [t']. inv H.
      exists t'. split; auto.
      eapply rtc_transitive.
      eapply reorder_postfix_subtree1; eauto.
      eapply rtc_transitive.
      eapply reorder_postfix_subtree2; eauto.
      assumption.
  - intros. inv H.
...
Inductive closed_tail_op a : Prop :=
  | ClosedTailOp ah ac :
      ClosedP ah ac →
      a ∈ ac →
      closed_tail_op a.

Inductive infix_tail_op a : Prop :=
  | InfixTailOp ah ac :
      InP ah ac →
      a ∈ ac →
      infix_tail_op a.

Inductive prefix_tail_op a : Prop :=
  | PrefixTailOp ah ac :
      PreP ah ac →
      a ∈ ac →
      prefix_tail_op a.

Inductive postfix_tail_op a : Prop :=
  | PostfixTailOp ah ac :
      PostP ah ac →
      a ∈ ac →
      postfix_tail_op a.
...
Inductive prefix_tail ac : Prop :=
  | PrefixTail ah ac2 :
      PreP ah ac2 →
      tail ac ac2 →
      prefix_tail ac.

Inductive postfix_tail ac : Prop :=
  | PostfixTail ah ac2 :
      PostP ah ac2 →
      tail ac ac2 →
      postfix_tail ac.
...
Create HintDb ops.
Hint Constructors closed_tail_op infix_tail_op prefix_tail_op postfix_tail_op
  tail closed_tail infix_tail prefix_tail postfix_tail
  mixfix_op mixfix_tail : ops.


Record noOverlap := mkNoOverlap {
  overlap1 : ∀ a ac1 ac2, ClosedP a ac1 → ClosedP a ac2 → ac1 = ac2;
  overlap2 : ∀ a ac1 ac2, InP a ac1 → InP a ac2 → ac1 = ac2;
  overlap3 : ∀ a ac1 ac2, PreP a ac1 → PreP a ac2 → ac1 = ac2;
  overlap4 : ∀ a ac1 ac2, PostP a ac1 → PostP a ac2 → ac1 = ac2;
  overlap5 : ∀ a ac, mixfix_op a → InP a ac → False;
  overlap6 : ∀ a ac, mixfix_op a → PostP a ac → False;
  overlap7 : ∀ a ac1 ac2, ClosedP a ac1 → PreP a ac2 → False;
  overlap8 : ∀ a ac1 ac2, InP a ac1 → PostP a ac2 → False;
}.

Create HintDb overlap.
Hint Resolve overlap1 overlap2 overlap3 overlap4
  overlap5 overlap6 overlap7 overlap8 : overlap.

Hypothesis NO : noOverlap.

Lemma ys_mixfix_deterministic w11 w12 w21 w22 a t1 t2 :
  mixfix_op a →
  w11 ++ a :: w12 = w21 ++ a :: w22 →
  ys w11 t1 →
  ys w21 t2 →
  w11 = w21
with yss_mixfix_deterministic w11 w12 w21 w22 a t1 t2 ti1 ti2 :
  mixfix_op a →
  w11 ++ a :: w12 = w21 ++ a :: w22 →
  yss ti1 w11 t1 →
  yss ti2 w21 t2 →
  w11 = w21
with il_mixfix_deterministic w11 w12 w21 w22 ac τ1 τ2 :
  mixfix_tail ac →
  w11 ++ w12 = w21 ++ w22 →
  il ac w11 τ1 →
  il ac w21 τ2 →
...
Proof.
  - intros HM. intros. inv H0; inv H1.
    + eapply overlap1 in H2 as ?; eauto. subst.
      ...
      eapply il_mixfix_deterministic with (w11 := wi) (w21 := wi0) in H3 as ?;
      eauto; [|inv HM; eauto with ops].
      subst. inv H1.
      eapply yss_mixfix_deterministic with (w11 := wt) (w21 := wt0) in H4; eauto.
      subst. auto. 
    + exfalso. eauto with overlap.
    + exfalso. eauto with overlap.
    + eapply overlap3 in H2 as ?; eauto. subst.
      repeat rewrite <- app_assoc in H1.
      eapply il_mixfix_deterministic with (w11 := wi) (w21 := wi0) in H3 as ?;
      eauto; [|inv HM; eauto with ops].
      subst. inv H1.
      eapply ys_mixfix_deterministic with (w11 := w) (w21 := w0) in H4; eauto.
      subst. reflexivity.
  - intros. inv H1; inv H2; auto.
    ...
    + repeat rewrite <- app_assoc in H2.
      eapply overlap2 in H3 as ?; eauto. subst.
      eapply il_mixfix_deterministic with (w11 := wi) (w21 := wi0) in H4 as ?; eauto with ops.
      subst. inv H2.
      eapply ys_mixfix_deterministic with (w11 := wt) (w21 := wt0) in H5 as ?; eauto.
      subst. reflexivity.
    + exfalso. eauto with overlap.
    + exfalso. eauto with overlap.
    + exfalso. eauto with overlap.
    + eapply overlap4 in H3 as ?; eauto. subst.
      repeat rewrite <- app_assoc in H2.
      eapply il_mixfix_deterministic with (w11 := wi) (w21 := wi0) in H4 as ?; eauto with ops.
      subst. inv H2.
      eapply yss_mixfix_deterministic with (w11 := wt) (w21 := wt0) in H5; eauto.
      subst. reflexivity.
  - intros. inv H1; inv H2; auto.
    ...
    assert (mixfix_op ah). {
      ...
      - inv H1. eapply MixfixOpClosed.
        eapply ClosedTailOp; eauto.
        eapply tail_subset; eauto.
        left.
      - inv H1. eapply MixfixOpInfix.
        eapply InfixTailOp; eauto.
        eapply tail_subset; eauto.
        left.
      - inv H1. eapply MixfixOpPrefix.
        eapply PrefixTailOp; eauto.
        eapply tail_subset; eauto.
        left.
      - inv H1. eapply MixfixOpPostfix.
        econstructor; eauto.
        eapply tail_subset; eauto.
        left.
    }
    eapply ys_mixfix_deterministic with (w11 := w1) (w21 := w0) in H3; eauto.
    subst. inv H0.
    ...
    eapply il_mixfix_deterministic with (w11 := w2) (w21 := w3) in H4; eauto.
    subst. auto.
Qed.
...
Proof.
  - intros. inv H; inv H0.
    + eapply overlap1 in H1 as ?; eauto. subst.
      ...
      eapply il_mixfix_deterministic in H2 as ?; eauto. subst.
      inv H4.
      eapply il_deterministic with (τ2 := τ0) in H2 as ?; eauto. subst.
      eapply yss_deterministic with (t2 := t2) in H3; eauto.
    + exfalso. eauto with overlap.
    + exfalso. eauto with overlap.
    + eapply overlap3 in H1 as ?; eauto. subst.
      eapply il_mixfix_deterministic in H2 as ?; eauto with ops. subst.
      inv H4.
      eapply il_deterministic with (τ2 := τ0) in H2; eauto with ops. subst.
      eapply ys_deterministic with (t2 := t0) in H3; eauto. subst.
      reflexivity.
  - intros. inv H; inv H0.
    + auto.
    + eapply overlap2 in H1 as ?; eauto. subst.
      eapply il_mixfix_deterministic in H4 as ?; eauto with ops.
      subst. inv H4.
    ...
    + exfalso. eauto with overlap.
    + exfalso. eauto with overlap.
    + eapply overlap4 in H1 as ?; eauto. subst.
      eapply il_mixfix_deterministic with (w11 := wi0) (w21 := wi) in H7 as ?; eauto with ops.
      subst. inv H4.
      eapply il_deterministic with (τ2 := τ0) in H2 as ?; eauto with ops. subst.
      eapply yss_deterministic with (t2 := t2) in H3; eauto.
  - intros. inv H0; inv H1; auto.
    assert (mixfix_op ah). {
      ...
      - inv H0. apply MixfixOpInfix.
        eapply InfixTailOp; eauto.
        eapply tail_subset; eauto. left.
      - inv H0. apply MixfixOpPrefix.
        eapply PrefixTailOp; eauto.
        eapply tail_subset; eauto. left.
      - inv H0. apply MixfixOpPostfix.
        econstructor; eauto.
        eapply tail_subset; eauto. left.
    }
    eapply ys_mixfix_deterministic in H2 as ?; eauto. subst.
    inv H5.
