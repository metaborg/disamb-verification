...
Inductive reorder_step : PT → PT → Prop :=
  ...
  | ReorderStepPrefixPostfix t τ a b :
      reorder_step (PeN a τ (PoN t b)) (PoN (PeN a τ t) b)
...
Proof.
  - intros. inv H.
    ...
    + simpl. rewrite <- app_assoc.
      edestruct yield_struct_postfix_sound; eauto. inv H.
      rename x into t'. exists (PeN a0 τ t'). split.
      * eapply PrefixYieldStruct; eauto.
...
Create HintDb ops.
Hint Constructors closed_tail_op infix_tail_op prefix_tail_op
  tail closed_tail infix_tail prefix_tail
...
Record noOverlap := mkNoOverlap {
  ...
  overlap4 : ∀ a ac, mixfix_op a → InP a ac → False;
  overlap5 : ∀ a, mixfix_op a → PostP a → False;
  overlap6 : ∀ a ac1 ac2, ClosedP a ac1 → PreP a ac2 → False;
  overlap7 : ∀ a ac, InP a ac → PostP a → False;
}.

Create HintDb overlap.
Hint Resolve overlap1 overlap2 overlap3 overlap4
  overlap5 overlap6 overlap7 : overlap.
...
Proof.
  ...
  - intros. inv H1; inv H2; auto.
    ...
    + exfalso. eauto with overlap.
    + exfalso. eauto with overlap.
    + eapply yss_mixfix_deterministic with (w11 := w) (w21 := w0) in H4; eauto.
      subst. reflexivity.
  - intros. inv H1; inv H2; auto.
